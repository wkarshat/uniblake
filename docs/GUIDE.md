    ...
    const unsigned char *salt, const unsigned char *personal)
{
  ...
  if (klen) {
    ...
    /* The key is absorbed as ONE ZERO-PADDED 128-byte block (RFC 7693 §3.3).
...
roughly 2.6x on the machine recorded in `TODO.md`.
...
If you are deciding whether uniblake clears secrets for you, this is the
answer.
...
### The unkeyed path does no key work at all

Worth stating because the reference does otherwise. In both uniblake
libraries the 128-byte key block is built **only** when a key is supplied:

- C: `ub_init` never touches it; only `ub_init_key` allocates the stack block,
  absorbs it, and burns it.
- Rust: `Params::to_state` passes an empty key and the block is inside
  `if !key.is_empty()`.

So an unkeyed digest pays no allocation, no 128-byte zero-fill, no extra
compression, and no wipe. There is nothing to skip because nothing is done.

`blake2b_simd` differs: `Params::key()` writes the key into a
`key_block: [u8; 128]` field of `Params` itself, so a keyed `Params` carries
that block for its whole lifetime and never clears it. An unkeyed `Params`
still carries the 128 zero bytes -- which is most of why its `Params` is 184
bytes against uniblake-rs's 34.

### Choosing

- Hashing public data (every observed observed call site): defaults are right,
